...
namespace BaseboardElectric {
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			BaseboardNum = FindItemInList( EquipName, Baseboard, []( BaseboardParams const& item ) { return item.EquipName; } );
			if ( BaseboardNum == 0 ) {
	...
	GetBaseboardInput()
	{
		...
		if ( NumConvElecBaseboards > 0 ) { //Get the data for cooling schemes
			...
			for ( ConvElecBBNum = 1; ConvElecBBNum <= NumConvElecBaseboards; ++ConvElecBBNum ) {
				...
				IsNotOK = false;
				IsBlank = false;
				VerifyName( cAlphaArgs( 1 ), Baseboard, []( BaseboardParams const& item ) { return item.EquipName; }, BaseboardNum, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
				if ( IsNotOK ) {
					ErrorsFound = true;
					continue;
				}
