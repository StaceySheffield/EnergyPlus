...
namespace PlantChillers {
	...
	)
	{
		...
		if ( SELECT_CASE_var == TypeOf_Chiller_Electric ) {
			...
			if ( CompIndex == 0 ) {
				ChillNum = FindItemInList( ChillerName, ElectricChiller, []( ElectricChillerSpecs const& item ) { return item.Base.Name; } );
				if ( ChillNum == 0 ) {
			...
			// Find the correct Chiller
			if ( CompIndex == 0 ) {
				ChillNum = FindItemInList( ChillerName, EngineDrivenChiller, []( EngineDrivenChillerSpecs const& item ) { return item.Base.Name; } );
				if ( ChillNum == 0 ) {
			...
			if ( CompIndex == 0 ) {
				ChillNum = FindItemInList( ChillerName, GTChiller, []( GTChillerSpecs const& item ) { return item.Base.Name; } );
				if ( ChillNum == 0 ) {
			...
			// Find the correct Chiller
			if ( CompIndex == 0 ) {
				ChillNum = FindItemInList( ChillerName, ConstCOPChiller, []( ConstCOPChillerSpecs const& item ) { return item.Base.Name; } );
				if ( ChillNum == 0 ) {
	...
	GetElectricChillerInput()
	{
		...
		for ( ChillerNum = 1; ChillerNum <= NumElectricChillers; ++ChillerNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), ElectricChiller, []( ElectricChillerSpecs const& item ) { return item.Base.Name; }, ChillerNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
	...
	GetEngineDrivenChillerInput()
	{
		...
		for ( ChillerNum = 1; ChillerNum <= NumEngineDrivenChillers; ++ChillerNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), EngineDrivenChiller, []( EngineDrivenChillerSpecs const& item ) { return item.Base.Name; }, ChillerNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
				if ( IsBlank ) cAlphaArgs( 1 ) = "xxxxx";
			}
	...
	GetGTChillerInput()
	{
		...
		for ( ChillerNum = 1; ChillerNum <= NumGTChillers; ++ChillerNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), GTChiller, []( GTChillerSpecs const& item ) { return item.Base.Name; }, ChillerNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
				if ( IsBlank ) cAlphaArgs( 1 ) = "xxxxx";
			}
	...
	GetConstCOPChillerInput()
	{
		...
		for ( ChillerNum = 1; ChillerNum <= NumConstCOPChillers; ++ChillerNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), ConstCOPChiller, []( ConstCOPChillerSpecs const& item ) { return item.Base.Name; }, ChillerNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
				if ( IsBlank ) cAlphaArgs( 1 ) = "xxxxx";
			}
