...
namespace BaseboardRadiator {
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			BaseboardNum = FindItemInList( EquipName, Baseboard, []( BaseboardParams const& item ) { return item.EquipID; } );
			if ( BaseboardNum == 0 ) {
	...
	GetBaseboardInput()
	{
		...
		if ( NumConvHWBaseboards > 0 ) { //Get the data for cooling schemes
			...
			for ( ConvHWBaseboardNum = 1; ConvHWBaseboardNum <= NumConvHWBaseboards; ++ConvHWBaseboardNum ) {
				...
				IsNotOK = false;
				IsBlank = false;
				VerifyName( cAlphaArgs( 1 ), Baseboard, []( BaseboardParams const& item ) { return item.EquipID; }, BaseboardNum, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
				if ( IsNotOK ) {
					ErrorsFound = true;
					continue;
				}
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			BaseboardNum = FindItemInList( BaseboardName, Baseboard, []( BaseboardParams const& item ) { return item.EquipID; } );
			if ( BaseboardNum == 0 ) {
