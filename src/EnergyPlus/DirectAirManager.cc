...
namespace DirectAirManager {
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			DirectAirNum = FindItemInList( EquipName, DirectAir, []( DirectAirProps const& item ) { return item.EquipID; } );
			if ( DirectAirNum == 0 ) {
	...
	GetDirectAirInput()
	{
		...
		if ( NumDirectAir > 0 ) {
			...
			for ( DirectAirNum = 1; DirectAirNum <= NumDirectAir; ++DirectAirNum ) {
				...
				IsNotOK = false;
				IsBlank = false;
				VerifyName( cAlphaArgs( 1 ), DirectAir, []( DirectAirProps const& item ) { return item.EquipID; }, DirectAirNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
				if ( IsNotOK ) {
					ErrorsFound = true;
