...
namespace Fans {
	...
	)
	{
		...
		if ( CompIndex == 0 ) {
			FanNum = FindItemInList( CompName, Fan, []( FanEquipConditions const& item ) { return item.FanName; } );
			if ( FanNum == 0 ) {
	...
	GetFanInput()
	{
		...
		for ( SimpFanNum = 1; SimpFanNum <= NumSimpFan; ++SimpFanNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), Fan, []( FanEquipConditions const& item ) { return item.FanName; }, FanNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
		...
		for ( NVPerfNum = 1; NVPerfNum <= NumNightVentPerf; ++NVPerfNum ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), NightVentPerf, []( NightVentPerfData const& item ) { return item.FanName; }, NVPerfNum - 1, IsNotOK, IsBlank, cCurrentModuleObject + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
				if ( IsBlank ) cAlphaArgs( 1 ) = "xxxxx";
			}
	...
	)
	{
		...
		FanIndex = FindItemInList( FanName, Fan, []( FanEquipConditions const& item ) { return item.FanName; } );
		if ( FanIndex == 0 ) {
	...
	)
	{
		...
		if ( present( FanIndex ) ) {
			...
			WhichFan = FindItemInList( FanName, Fan, []( FanEquipConditions const& item ) { return item.FanName; } );
			if ( WhichFan != 0 ) {
