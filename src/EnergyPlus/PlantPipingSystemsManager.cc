...
namespace PlantPipingSystemsManager {
	...
	)
	{
			...
			for ( ZoneCoupledDomainCtr = 1; ZoneCoupledDomainCtr <= NumZoneCoupledDomains; ++ZoneCoupledDomainCtr ) {
				...
				IsNotOK = false;
				IsBlank = false;
				VerifyName( cAlphaArgs( 1 ), Domain, []( GroundDomainData const& item ) { return item.ObjName; }, ZoneCoupledDomainCtr - 1, IsNotOK, IsBlank, ObjName_ZoneCoupled_Slab + " Name" );
				if ( IsNotOK ) {
					ErrorsFound = true;
					cAlphaArgs( 1 ) = "Duplicate name encountered";
				} else if ( IsBlank ) {
				...
				if ( PipingSystemDomains( DomainCtr ).SlabInGradeFlag ) {
					...
					PipingSystemDomains( DomainCtr ).SlabMaterialNum = FindItemInList( cAlphaArgs( 4 ), Material );
	...
	)
	{
		...
		for ( BasementCtr = 1; BasementCtr <= NumBasements; ++BasementCtr ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), Domain, []( GroundDomainData const& item ) { return item.ObjName; }, BasementCtr - 1, IsNotOK, IsBlank, ObjName_ZoneCoupled_Basement + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
				cAlphaArgs( 1 ) = "Duplicate name encountered";
			} else if ( IsBlank ) {
	...
	)
	{
		...
		for ( HorizontalGHXCtr = 1; HorizontalGHXCtr <= NumHorizontalTrenchesInInput; ++HorizontalGHXCtr ) {
			...
			IsNotOK = false;
			IsBlank = false;
			VerifyName( cAlphaArgs( 1 ), HGHX, []( HorizontalTrenchData const& item ) { return item.ObjName; }, HorizontalGHXCtr - 1, IsNotOK, IsBlank, ObjName_HorizTrench + " Name" );
			if ( IsNotOK ) {
				ErrorsFound = true;
				cAlphaArgs( 1 ) = "Duplicate name encountered";
			} else if ( IsBlank ) {
