	...
	SetUpZoneSizingArrays()
	{
		...
		for ( ZoneSizIndex = 1; ZoneSizIndex <= NumZoneSizingInput; ++ZoneSizIndex ) {
			...
			if ( any( ZoneEquipConfig.IsControlled() ) ) {
				ZoneIndex = FindItemInList( ZoneSizingInput( ZoneSizIndex ).ZoneName, ZoneEquipConfig, []( EquipConfiguration const& item ) { return item.ZoneName; } );
				if ( ZoneIndex == 0 ) {
		...
		for ( DesDayNum = 1; DesDayNum <= TotDesDays + TotRunDesPersDays; ++DesDayNum ) {
			for ( CtrlZoneNum = 1; CtrlZoneNum <= NumOfZones; ++CtrlZoneNum ) {
				...
				ZoneSizNum = FindItemInList( ZoneEquipConfig( CtrlZoneNum ).ZoneName, ZoneSizingInput, []( ZoneSizingInputData const& item ) { return item.ZoneName; } );
